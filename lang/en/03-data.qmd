...
#| echo: false
unicorns <- read.table("data/unicorns.txt", header = TRUE, stringsAsFactors = TRUE)
knitr::kable(rbind(head(unicorns), tail(unicorns)), row.names = FALSE)
```
...
#| message: false
library(kableExtra)
unicorns <- read.table("data/unicorns.txt", header = TRUE)
knitr::kable(rbind(head(unicorns), tail(unicorns)), row.names = FALSE, "latex", booktabs = T) %>%
...
If you want to follow along you can download the data file (*'unicorn.xls'*) from @sec-data-files.
...
Let's import a tab delimited file called `unicorns.txt` which contains the data we saw previously in this Chapter (@sec-df) and assign it to an object called `unicorns`.
...
#| echo: true
unicorns <- read.table(
  file = "data/unicorns.txt", header = TRUE, sep = "\t",
  stringsAsFactors = TRUE
)
```
...
#| eval: false
unicorns <- read.table(
  file = "C:\\Documents\\Prog1\\data\\unicorns.txt",
...
#| echo: true
unicorns <- read.table(
  file = "data/unicorns.txt", header = TRUE,
...
unicorns <- read.csv2(file = "data/unicorns.csv")
...
For example, to extract the first 8 rows and all of the columns in the `unicorns` data frame
...
#| eval: !expr knitr::is_latex_output()
#| warning: false
rbind(head(unicorns[, 1:3], n = 10), tail(unicorns[, 1:3], n = 10))
```
