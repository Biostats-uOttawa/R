# Reproducible reports with R markdown / Quarto {#sec-rmarkdown_r}
...
* the different components of an R markdown document
* how to format text, graphics and tables within the document
* how to avoid some of the common difficulties using R markdown.
...
Quarto uses `.qmd` files while R markdown works with `.qmd` but Quarto can render `.qmd` files too.
...
Your data and Quarto file can be included with your publication or thesis chapter as supplementary material or hosted on a GitHub repository (see @sec-github_r).
...
You can find instructions on how to do this in @sec-install-rm and on the [Quarto website](https://quarto.org/docs/get-started/).
...
Again, instructions on how to do this can be found at @sec-install-rm.

### Create a Quarto document, `.qmd` {#sec-new_rm}
...
If everything went smoothly a new HTML file will have been created and saved in the same directory as your `.qmd` file (ours will be called `my_first_rmarkdown.html`).
...
quarto_render('my_first_rmarkdown.qmd', output_format = 'html_document')

# alternatively if you don't want to load the quarto package

quarto::quarto_render('my_first_rmarkdown.Rmd', output_format = 'html_document')
```
...
In RStudio a minimal YAML header is automatically created for you when you create a new Quarto document as we did above (@sec-new_rm) but you can change this any time.
...
Interestingly there is no underline in R markdown syntax by default, for multiple more or less esoteric reasons (*e.g.* an underline is considered a stylistic element (there may well be other [reasons][underline])).
...
You can, however, use a `#` symbol to comment code inside a code chunk (@sec-code_chunks) as usual (more about this in a bit).
...
The code above will only work if the image file (`Cute_grey_kitten.jpg`) is in the right place relative to where you saved your `.qmd` file.
In the example above the image file is in a sub directory (folder) called `images` in the directory where we saved our `my_first_rmarkdown.qmd` file.
...
However, to do this you will need to include this R code in a `code chunk` (@sec-code_chunks) which we haven't covered yet.
...
#| fig-align: center
#| out-width: 50%
knitr::include_graphics("images/Cute_grey_kitten.jpg")
```
...
Quarto can print the contents of a data frame as a table (or any other tabular object such as a summary of model output) by including the name of the data frame in a code chunk.
...
To create a table of the first 10 rows of the `iris` data frame using the `kable()` function simply write your code block as

```{r}
#| label: kable-table-html
#| echo: fenced
...
```{r}
#| label: kable-table-latex
#| echo: false
#| eval: !expr knitr::is_latex_output()
#| warning: false
library(knitr)
kable(iris[1:10,], "latex", booktabs = TRUE)
```
...
```{r}
#| label: kable-table2-latex
#| echo: fenced
#| eval: !expr knitr::is_latex_output()
#| warning: false
library(knitr)
kable(iris[1:10,], "latex", booktabs = TRUE, digits=0, align = 'c', col.names = c('sepal length', 'sepal width', 'petal length', 'petal width', 'species'))
```
...
## Some tips and tricks {#sec-tips_tricks}
