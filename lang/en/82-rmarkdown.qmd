...
#| fig-align: center
#| out-width: 85%
include_graphics("images/markdown/Rmarkdown_console.png")
```
...
In the example the image file is in a sub directory (folder) called `images/markdown` in the directory where we saved our `my_first_rmarkdown.qmd` file.
...
#| warning: false
library(knitr)
kable(iris[1:5,], "latex", booktabs = TRUE)
```
...
#| warning: false
library(knitr)
kable(iris[1:5,], "latex", booktabs = TRUE, digits=0, align = 'c', col.names = c('sepal length', 'sepal width', 'petal length', 'petal width', 'species'))
```
...
#| warning: false
library(kableExtra)
kable(iris[1:5,],"latex", booktabs = T) %>%
