...
`ggplot2`\index{ggplot2 package} was based on a book called *Grammar of Graphics* by @wilksinon_grammar_2005a. For an interesting summary of Wilkinson's book [here][leland-summary]. The *Grammar of Graphics* approach breaks figures down into their various components (e.g. the underlying statistics, the geometric arrangement, the theme, see @ref(fig:ggplot2_fw)). Users are thus able to manipulate each of these components (i.e. layers) and produce a tailor-made figure fit for their specific needs.
...
```{r ggplot2_fw, echo=FALSE, out.width="50%", fig.align="center", fig.cap = "framework behind ggplot2"}
...
To redo the figure @ref(fig:plot5), that contain only a scatterplot of point we can use the `geom_point()` function.

```{r plot5_gg, out.width = "75%", fig.align = "center", message=FALSE}
ggplot(
  data = flowers,
...
Simple type of plots really useful to have a look at the relation between 2 variables for example. Here are the code to do it using either base R @ref(fig:plot5) 

```{r plot5, eval = FALSE}
```
 or ggplot @ref(fig:plot5_gg)
```{r plot5_gg, eval = FALSE}
```
...
```{r plot5_gg_line, out.width = "75%", fig.align = "center", message=FALSE}
ggplot(
  data = flowers,
  mapping = aes(x = weight, y = shootarea)
) +
...
with ggplot2
```{r plot7_gg, echo=TRUE, out.width="75%", fig.align="center"}
...
```{r plot8_gg, echo=TRUE, out.width="75%", fig.align="center"}
brk <- seq(from = 0, to = 18, by = 1)
ggplot(flowers, aes(x = height)) +
...
```{r plot10_gg, echo=TRUE, out.width="75%", fig.align="center"}
ggplot(flowers, aes(x = height)) +
  geom_histogram(aes(y = after_stat(density)), breaks = brk) +
  geom_density() +
  ggtitle("petunia height")
...
```{r plot11_gg, out.width = "75%", fig.align = "center", message=FALSE}
...
```{r plot12_gg, out.width = "75%", fig.align = "center", message=FALSE}
...
```{r plot14_gg, echo=TRUE, out.width="75%", fig.align="center"}
ggplot(flowers, aes(y = weight, x = nitrogen)) +
  geom_boxplot() +
  labs(y = "Weight (g)", x = "Nitrogen Concentration") +
  facet_grid(. ~ treat)
...
```{r plot16_gg, echo=TRUE, out.width="75%", fig.align="center", warning=FALSE, message=FALSE}
ggplot(flowers, aes(y = weight, x = nitrogen)) +
  geom_violin() +
...
<!-- TO DO -->
```{r plot19_gg, echo=TRUE, out.width="75%", fig.align="center"}
...
All of the plots we've created so far in this Chapter are more than suitable for exploring your data. If however, you'd like to make them a little prettier (for your thesis, publication or even your own amusement) you'll need to invest some time learning how to customise your plots. Given that we think `ggplot` is a much more convenient way of customizing and exporting plots, we only provide code using ggplot. For information on how to do this using base R see the excellent info [here](https://intro2r.com/custom_plot.html). 
...
In base R, one of the most common methods to plot multiple graphs is to use the main graphical function `par()`\index{par(mfrow=)} to split the plotting device up into a number of defined sections using the `mfrow =` argument. With this method, you first need to specify the number of rows and columns of plots you would like and then run the code for each plot. For example, to plot two graphs side by side we would use `par(mfrow = c(1, 2))` to split the device into 1 row and two columns.
