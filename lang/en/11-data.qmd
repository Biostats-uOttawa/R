...
Enter the file name (keep it fairly short without spaces (@sec-file_names)!) in the 'Save as:' dialogue box.
...
Let's import a tab delimited file called `flower.txt` which contains the data we saw previously in this Chapter (@sec-df) and assign it to an object called `flowers`.
The file is located in a `data` directory which itself is located in our root directory (@sec-dir_struct).
...
Using RStudio Projects (@sec-rsprojs) and having a logical directory structure (@sec-dir_struct) goes a long way to avoiding these types of errors.
