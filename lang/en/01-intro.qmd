# Getting started {#chap1}

Although R is not new, its popularity has increased rapidly over the last 10 years or so (see [here][r4stats] for some interesting data).
...
So, why has R become so popular and why should you learn how to use it? Some reasons include:
...
- R has an expanding set of freely available ‘packages’ `r emoji::emoji("package")` to extend R's capabilities.
...
However, in our opinion, the single biggest reason to use R is that it facilitates robust and reproducible research practices.
...
You are then able to share this code (and your data) with other researchers / colleagues / journal reviewers who will be able to reproduce your analysis exactly.
This is one of the tenets of [open science][open-sci].
We will cover other topics to facilitate open science throughout this book, including creating [reproducible reports](#rmarkdown_r) and [version control](#github_r).
...
A plot of the numbers 1 to 10 on both the x and y axes should appear.
If you see this, you're good to go.
If not then we suggest you make a note of any errors produced and then use Google to troubleshoot.
...
::: {.callout-caution}
You must install R before you install an IDE (see [previous section](#install_r) for details).
:::
...
- [httpgd](https://cran.r-project.org/web/packages/httpgd/index.html): An R package `r emoji::emoji("package")` to provide a graphics device that asynchronously serves SVG graphics via HTTP and WebSockets.
...
However, we would suggest that you don't use this approach to import your data as it's not reproducible and therefore not robust (see [Chapter 3](#data_r) for more details).
...
Plots can be exported in various file formats such as jpeg, png, pdf, tiff or copied to the clipboard (although you are probably better off using the appropriate R functions to do this - see [Chapter 4](#graphics_r) for more details).
...
We will go over how to view the help files and how to search for help in [Chapter 2](#basics_r).
...
![file](images/ch-01/vs_file_panel.png){#fig-vsfile}
...
You first need to install the `BiocManager` `r emoji::emoji("package")` package.
...
Once the `BiocManager``r emoji::emoji("package")` package has been installed you can either install all of the 'core' Bioconductor packages with

```{r, echo=TRUE, eval=FALSE}
BiocManager::install()
```

or install specific packages such as the `GenomicRanges` `r emoji::emoji("package")`  and `edgeR` `r emoji::emoji("package")` packages
...
Perhaps the most efficient method is to use the `install_github()` function from the `remotes` `r emoji::emoji("package")` package (you installed this package [previously](#cran_packages)).
...
For example, the development version of `dplyr` `r emoji::emoji("package")` from Hadley Wickham is hosted on the tidyverse GitHub account and has the repository name 'dplyr' (just Google 'github dplyr').
...
For example, to load the `remotes` `r emoji::emoji("package")`  package you previously installed
...
For example, if you try to use the `install_github()` function without loading the `remotes` `r emoji::emoji("package")` package first you will receive the following error
...
This is how we were able to use the `install()` and `install_github()` functions [above][Bioconductor packages] without first loading the packages `BiocManager` `r emoji::emoji("package")`  and `remotes` `r emoji::emoji("package")` .
...
For example, let's say that you've created a subdirectory called `data` in your Root project directory that contains a csv delimited datile called `mydata.csv` (we will cover directory structures [below](#dir_struct)).
To import this datile in an RStudio project using the `read.csv()` function (don't worry about this now, we will cover this in much more detail in [Chapter 3](#data_r)) all you need to include in your R script is
...
You can create either an entirely new project, a project from an existing directory or a version controlled project (see the [GitHub Chapter](#github_r) for further details about this).
...
Now that you have an RStudio project set up you can start creating R scripts (or [R markdown](#rmarkdown_r) documents)  or whatever you need to complete you project.
...
What you call your files matters more than you might think.
...
Although there's not really a recognized standard approach to naming files (actually [there is][file_wiki], just not everyone uses it), there are a couple of things to bear in mind.
  ...
  - First, avoid using spaces in file names by replacing them with underscores or even hyphens.
...
Take R markdown for example, if you want to render an R markdown document to pdf using the `rmarkdown` `r emoji::emoji("package")` package you will actually be using a command line LaTeX engine under the hood (called [Pandoc][pandoc]).
Another good reason not to use spaces in file names is that it makes searching for file names (or parts of file names) using [regular expressions][regex] in R (or any other language) much more difficult.
  ...
  - For the reasons given above, also avoid using special characters (i.e.
@£$%^&*(:/) in your file names.
 
  - If you are versioning your files with sequential numbers (i.e.
file1, file2, file3 ...) and you have more than 9 files you should use 01, 02, 03 ..
10 as this will ensure the files are printed in the correct order (see what happens if you don't).
If you have more than 99 files then use 001, 002, 003...
etc.
...
This information could be included as a series of comments `#` or, even better, by mixing executable code with narrative into an [R markdown](#rmarkdown_r) document.
...
A really good alternative is to use the `session_info()` function from the `xfun` `r emoji::emoji("package")` package for a more concise summary of our session environment.
...
RStudio will magically convert `ts` into the current date and time and also automatically comment out this line with a `#`.
...
- RStudio you can install the `styler` `r emoji::emoji("package")` package which includes an RStudio add-in to allow you to automatically restyle selected code (or entire files and projects) with the click of your mouse.
You can find more information about the `styler` `r emoji::emoji("package")` package including how to install [here][styler].
...
Now highlight the code and use the `styler` `r emoji::emoji("package")` package to reformat
...
Don't be that person who thinks it'll never happen to me - it will! Always think of the absolute worst case scenario, something that makes you wake up in a cold sweat at night, and do all you can to make sure this never happens.
...
To find out more about how you can use RStudio, Git and GitHub together see the Git and GitHub [Chapter](#github_r).
       

## Citing R

Many people have invested huge amounts of time and energy making R the great piece of software you're now using.
If you use R in your work (and we hope you do) please remember to give appropriate credit by citing R.
...
To cite multiple packages, the package `grateful` `r emoji::emoji("package")` is extermely useful. See the [reference section](98-references.qmd) for an example output produced with grateful.
