...
- R has an expanding set of freely available ‘packages’ :package: to extend R's capabilities.
...
Perhaps the most efficient method is to use the `install_github()` function from the `remotes` :package: package (you installed this package [previously](#cran_packages)).
...
For example, the development version of `dplyr` :package: from Hadley Wickham is hosted on the tidyverse GitHub account and has the repository name 'dplyr' (just Google 'github dplyr').
...
Take R markdown for example, if you want to render an R markdown document to pdf using the `rmarkdown` :package: package you will actually be using a command line LaTeX engine under the hood (called [Pandoc][pandoc]).
...
To cite multiple packages, the package `grateful` :package: is extermely useful.
