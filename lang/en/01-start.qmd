...
We will cover other topics to facilitate open science throughout this book, including creating reproducible reports (@sec-rmarkdown_r) and version control (@sec-github_r).
...
::: {.callout-caution}
You must install R before you install an IDE (see @sec-install_r for details).
:::
...
However, we would suggest that you don't use this approach to import your data as it's not reproducible and therefore not robust (see @sec-data_r for more details).
...
Plots can be exported in various file formats such as jpeg, png, pdf, tiff or copied to the clipboard (although you are probably better off using the appropriate R functions to do this - see [@sec-graphics_r for more details).
...
We will go over how to view the help files and how to search for help in @sec-basics_r).
...
For example, let's say that you've created a subdirectory called `data` in your Root project directory that contains a csv delimited datile called `mydata.csv` (we will cover directory structures below in @sec-dir_struct).
To import this datile in an RStudio project using the `read.csv()` function (don't worry about this now, we will cover this in much more detail in @sec-data_r) all you need to include in your R script is
...
## Directory structure {#sec-dir_struct}
...
You can create either an entirely new project, a project from an existing directory or a version controlled project (see the @sec-github_r for further details about this).
...
Now that you have an RStudio project set up you can start creating R scripts (or R markdown /Quarto documents, see @sec-rmarkdown_r)  or whatever you need to complete you project.
...
## Script documentation {#sec-proj_doc}
...
This information could be included as a series of comments `#` or, even better, by mixing executable code with narrative into an R markdown document (@sec-rmarkdown_r).
...
To find out more about how you can use RStudio, Git and GitHub together see @sec-github_r.
