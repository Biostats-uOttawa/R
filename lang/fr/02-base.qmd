...
Les résultats de la recherche sont affichés dans RStudio sous l'onglet "Aide" comme précédemment. `help.search()` recherche dans la documentation d'aide, les démonstrations de code et les vignettes de paquetage et affiche les résultats sous forme de liens cliquables pour une exploration plus approfondie.
...
Cependant, l'inconvénient potentiel de l'utilisation de la version de développement d'un paquetage R est qu'elle peut ne pas être aussi stable que la version hébergée sur CRAN (elle est en cours de développement !) et que la mise à jour des paquets ne sera pas automatique.
...
Si vous essayez d'utiliser une fonction sans avoir préalablement chargé le paquetage R correspondant, vous recevrez un message d'erreur indiquant que R n'a pas pu trouver la fonction.
Par exemple, si vous essayez d'utiliser la fonction `install_github()` sans charger le paquetage `remotes` :package : en premier lieu, vous obtiendrez l'erreur suivante
