...
1. vous présenter R, un environnement interactif puissant et flexible pour le calcul et la recherche statistiques.
    pour le calcul et la recherche statistiques.
2. vous présenter (ou vous familiariser à nouveau) avec l'analyse statistique effectuée
    dans R.

R n'est pas difficile à apprendre en soi, mais comme pour toute nouvelle langue (parlée ou informatique), la courbe d'apprentissage initiale peut être difficile à suivre.
langue (parlée ou informatique), la courbe d'apprentissage initiale peut être raide
...
(ni avec R, ni avec les statistiques), mais simplement de vous aider à gravir la courbe d'apprentissage
la courbe d'apprentissage initiale (potentiellement plus rapide) et à vous fournir les
compétences de base (et la confiance !) nécessaires pour commencer votre propre voyage avec R et des analyses spécifiques.
avec des analyses spécifiques.
...
La première partie du livre sur l'utilisation de R a commencé comme un fork sur github à partir de l'excellent livre de Douglas, Roos, Mancini.
[An introduction to R](https://intro2r.com/) par Douglas, Roos, Mancini,
...
Site qui a fourni beaucoup d'informations pour le livre :

- [Introduction à dplyr](https://dplyr.tidyverse.org/articles/dplyr.html)
- Introduction à gam](https://m-clark.github.io/generalized-additive-models/)
