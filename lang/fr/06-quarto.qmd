...
Dans le bloc suivant, nous nommons le morceau de code `summary-stats`, nous chargeons le paquetage `ggplot2` `r emoji::emoji("package")` créer un cadre de données (`dataf`) avec deux variables `x` et `y`, utiliser la méthode `summary()` pour afficher des statistiques sommaires et tracer un nuage de points des données à l'aide de la fonction `ggplot()`.
...
Une autre façon d'inclure des images dans votre document est d'utiliser la fonction `include_graphics()` de la fonction `knitr` du paquetage. Le code suivant produira un résultat similaire.
