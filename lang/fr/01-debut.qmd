...
- Utilisez R souvent et régulièrement.
  Cela permettra de créer et de maintenir une dynamique importante.
...
- Reconnaissez qu'il y aura des moments où les choses deviendront un peu difficiles ou frustrantes.
...
Il est fortement recommandé d'utiliser un **I**ntégrée **D**éveloppement **E**nvironnement de développement (IDE) pour travailler avec R.
...
VSCode est disponible gratuitement pour les systèmes d'exploitation Windows, Mac et Linux et peut être téléchargé à partir du [VS Code][vscode] .

En outre, vous devez installer le logiciel [R pour VSCode](https://marketplace.visualstudio.com/items?itemName=REditorSupport.r).
...
- [VSCode-R-Debugger](https://marketplace.visualstudio.com/items?itemName=RDebugger.r-debugger) VSCode-R-Debugger : Une extension de VS Code pour supporter les capacités de débogage de R.
- [httpgd](https://cran.r-project.org/web/packages/httpgd/index.html) httpgd : Un paquetage R :paquetage : pour fournir un dispositif graphique qui sert de manière asynchrone des graphiques SVG via HTTP et WebSockets.
...
Si vous ne savez pas ce qu'est un éditeur de script, vous pouvez l'assimiler à un traitement de texte, mais spécialement conçu pour écrire du code.
Heureusement, de nombreux éditeurs de scripts sont disponibles gratuitement. N'hésitez donc pas à les télécharger et à les tester jusqu'à ce que vous en trouviez un qui vous convienne.
...
C'est à vous de choisir celui qui vous convient le mieux : l'une des grandes qualités de R est que *VOUS* c'est que VOUS pouvez choisir comment vous voulez utiliser R.
...
- [Atom][atome] (tous les systèmes d'exploitation)
...
La grande fenêtre (ou volet) de gauche est le module **la console** de la console.
La fenêtre en haut à droite est la fenêtre **Environnement / Histoire / Connexions** et la fenêtre en bas à droite est le volet **Fichiers / Tracés / Paquets / Aide / Visionneuse** .
...
Par exemple, si vous tapez `2 + 2` dans la console, vous devriez obtenir la réponse `4` (rassurante).
...
Un script R est un simple fichier texte avec une balise `.R` qui contient vos lignes de code R.
...
- L'onglet "Environnement" affiche tous les objets que vous avez créés dans l'environnement actuel (global).
  ...
  Les objets peuvent être affichés sous forme de liste ou de grille en sélectionnant votre choix dans le bouton déroulant situé en haut à droite de la fenêtre.
  Si vous utilisez le format Grille, vous pouvez supprimer des objets de l'environnement en cochant la case vide située à côté du nom de l'objet, puis en cliquant sur l'icône représentant un balai.
...
- L'onglet "Tracés" est l'endroit où tous les tracés que vous créez dans R sont affichés (sauf indication contraire de votre part).
  Vous pouvez "zoomer" sur les tracés pour les agrandir à l'aide du bouton de la loupe et faire défiler les tracés créés précédemment à l'aide des boutons fléchés.
  Il est également possible d'exporter les tracés vers un fichier externe à l'aide du menu déroulant "Export".
  Les tracés peuvent être exportés dans différents formats de fichiers tels que jpeg, png, pdf, tiff ou copiés dans le presse-papiers (bien qu'il soit probablement préférable d'utiliser les fonctions R appropriées pour ce faire - voir \[@sec-graphics-r pour plus de détails).

- L'onglet "Packages" répertorie tous les packages que vous avez installés sur votre ordinateur.
...
- L'onglet "Aide" affiche la documentation d'aide R pour chaque fonction.
  Nous verrons comment consulter les fichiers d'aide et comment rechercher de l'aide dans @sec-basics-r).

- L'onglet "Viewer" affiche le contenu web local tel que les graphiques web générés par certains packages.
...
- un tableau de bord (avec historique et navigation)
- édition de scripts
...
Cependant, le problème avec `setwd()` est qu'il utilise un *absolue* spécifique à l'ordinateur sur lequel vous travaillez.
...
Le répertoire du projet racine est simplement le répertoire qui contient le fichier `.Rproj` dans Rstudio (`first_project.Rproj` dans notre cas) ou le dossier de base de votre espace de travail dans VScode.
Si vous souhaitez partager votre analyse avec quelqu'un d'autre, il vous suffit de copier le répertoire complet du projet et de l'envoyer à votre collaborateur.
...
Pour ceux d'entre vous qui souhaitent pousser plus loin la notion de chemins d'accès relatifs aux fichiers, jetez un coup d'œil à l'option `here()` de la fonction `here` [paquet][ici] .
Les `here()` vous permet de créer des chemins d'accès à n'importe quel fichier par rapport au répertoire racine du projet, qui ne dépendent pas du système d'exploitation (fonctionne sur une machine Mac, Windows ou Linux).
...
- **Racine** - Il s'agit du répertoire de votre projet contenant votre `.Rproj` fichier.
...
- **données** - Nous stockons toutes nos données dans ce répertoire.
  ...
  Toutes les données traitées doivent être sauvegardées dans un fichier séparé et stockées dans le fichier `processed_data` .
  Les informations sur les méthodes de collecte des données, les détails du téléchargement des données et toute autre métadonnée utile doivent être sauvegardés dans un document texte (voir les fichiers README ci-dessous) dans le sous-répertoire `metadata` dans le sous-répertoire
...
- **scripts** - Un répertoire optionnel où nous enregistrons nos documents R markdown et/ou les principaux scripts R que nous avons écrits pour le projet en cours sont enregistrés ici si ce n'est pas dans le dossier racine.
...
Vous pouvez créer votre structure de répertoire à l'aide de l'explorateur Windows (ou du Finder sur Mac) ou dans votre IDE en cliquant sur le bouton "Nouveau dossier" dans le panneau "Fichiers".
...
Un bon moyen de garder les choses organisées est d'utiliser les projets RStudio ou les espaces de travail VSCode, désignés sous le nom de `project`.
A `project` conserve tous vos scripts R, vos documents R markdown, vos fonctions R et vos données en un seul endroit.
Ce qu'il y a de bien avec `project` est que chacun a son propre répertoire, son propre historique et ses propres documents sources, de sorte que les différentes analyses sur lesquelles vous travaillez sont complètement séparées les unes des autres.
Cela signifie que vous pouvez très facilement passer d'une analyse à l'autre. `projects` sans craindre qu'elles n'interfèrent l'une avec l'autre.

### RStudio

Pour créer un projet, ouvrez RStudio et sélectionnez `File` -> `New Project...` dans le menu.
...
Saisissez le nom du répertoire que vous souhaitez créer dans le champ "Nom du répertoire :" (nous l'appellerons `first_project` pour ce chapitre).
...
Une fois votre nouveau projet créé, vous disposerez d'un nouveau dossier sur votre ordinateur contenant un fichier de projet RStudio appelé `first_project.Rproj`.
...
La dernière chose que nous vous suggérons de faire est de sélectionner `Tools` -> `Project Options...` dans le menu.
...
Prenez R markdown par exemple, si vous voulez rendre un document R markdown au format pdf en utilisant le paquetage `rmarkdown` :package : vous utiliserez en fait une ligne de commande {{< latex >}} sous le capot.
...
Un raccourci très utile dans RStudio est d'inclure automatiquement un horodatage dans votre script R. Pour ce faire, écrivez `ts` à l'endroit où vous souhaitez insérer votre horodatage dans votre script R.
Pour ce faire, écrivez `ts` à l'endroit où vous souhaitez insérer votre horodateur dans votre script R, puis appuyez sur les touches 'shift + tab'.
...
Pour ce faire, mettez en surbrillance les lignes de texte que vous souhaitez commenter et appuyez sur 'ctrl + shift + c' (ou 'cmd + shift + c' sur un mac).
...
- Pour RStudio, vous pouvez installer l'extension `styler` :package : qui inclut un complément RStudio vous permettant de reformater automatiquement le code sélectionné (ou des fichiers et projets entiers) d'un simple clic de souris.
  Vous pouvez trouver plus d'informations sur le paquet :package :. `styler` :package :, y compris comment l'installer [ici][styler] .
  Une fois installé, vous pouvez mettre en évidence le code que vous souhaitez remodeler, cliquer sur le bouton "Addins" en haut de RStudio et sélectionner l'option "Style Selection".
...
Mettez maintenant le code en surbrillance et utilisez la commande `styler` :package : package pour reformater
...
Ne soyez pas cette personne qui pense que cela ne lui arrivera jamais - cela lui arrivera !
Pensez toujours au pire scénario, à quelque chose qui vous donne des sueurs froides la nuit, et faites tout ce qui est en votre pouvoir pour que cela n'arrive jamais.
Pour être clair, si vous comptez copier vos précieux fichiers sur un disque dur externe ou une clé USB, il est préférable de ne pas le faire. **PAS** une stratégie de sauvegarde efficace.
Ces objets tombent souvent en panne lorsque vous les glissez dans votre sac à dos ou votre "sac à vie" et que vous les trimballez entre votre bureau et votre domicile.
...
Pour retrouver les versions précédentes des fichiers, il faut souvent passer un temps fou à parcourir plusieurs fichiers nommés *'final.doc'*, *'final_v2.doc'* et *'final_usethisone.doc'* etc. jusqu'à ce que vous trouviez celui que vous cherchiez.
...
Pour obtenir la citation la plus récente pour R, vous pouvez utiliser le site web `citation()` fonction.
...
Selon nous, l'outil le plus utile pour la citation est le package `grateful` :package : qui vous permet de générer les informations de citation dans un fichier, ainsi que de créer une phrase ou un tableau citant tous les packages utilisés. Cela devrait devenir la norme dans tout manuscrit honnêtement.
