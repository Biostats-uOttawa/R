...
- [httpgd](): An R package `r emoji::emoji("package")` to provide a graphics device that asynchronously serves SVG graphics via HTTP and WebSockets.
...
You first need to install the `BiocManager` package.
...
Perhaps the most efficient method is to use the `install_github()` function from the `remotes` package (you installed this package [previously](#cran_packages)).
...
For example, the development version of `dplyr` from Hadley Wickham is hosted on the tidyverse GitHub account and has the repository name 'dplyr' (just Google 'github dplyr').
...
It is important to realise that every time you start a new R session (or restore a previously saved session) you need to load the packages you will be using.
...
For example, if you try to use the `install_github()` function without loading the `remotes` package first you will receive the following error
...
This is how we were able to use the `install()` and `install_github()` functions [above][Bioconductor packages] without first loading the packages `BiocManager` and `remotes`.
...
As with most things in life, when it comes to dealing with data and data analysis things are so much simpler if you're organised.
...
In addition, it's much easier to write code to automate tasks when files are well organised and are sensibly named.
...
Lastly, having a well organised project reduces the risk of introducing bugs or errors into your workflow and if they do occur (which inevitably they will at some point), it makes it easier to track down these errors and deal with them efficiently.
...
Take R markdown for example, if you want to render an R markdown document to pdf using the `rmarkdown` package you will actually be using a command line LaTeX engine under the hood (called [Pandoc][pandoc]).
...
A really good alternative is to use the `session_info()` function from the `xfun` package for a more concise summary of our session environment.
...
- RStudio you can install the `styler` package which includes an RStudio add-in to allow you to automatically restyle selected code (or entire files and projects) with the click of your mouse.
You can find more information about the `styler` package including how to install [here][styler].
